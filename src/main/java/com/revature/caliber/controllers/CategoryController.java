...
/**	@ApiOperation(value = "Get a Category by its id", response = Category.class)
	...
	/**
	 * Soft delete by id. Sets the Category to inactive
	 * 
	 * @param id - Category's id
	 * @return a Category with an updated state along with HTTP status code 202 (ACCEPTED)
	 */
/**	@ApiOperation(value = "Finds a Category by id and sets it to active or inactive", response = Category.class)
	@DeleteMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Category> deactivateByCategoryId(@PathVariable int id) {
		Category category = this.cs.findByCategoryID(id);

		if (category != null) {
			category.setIsActive(false);
			category = cs.saveCategory(category);
		}
...
/**	@ApiOperation(value = "Gets a list of all active categories", response = Category.class, responseContainer = "List")
	...
	public ResponseEntity<List<Category>> findAllActive() {
		List<Category> categories = this.cs.findAllActive();
